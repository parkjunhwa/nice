...
export default function Rul002Page() {
  ...
  const [monthlyFixedAmount, setMonthlyFixedAmount] = useState('200000')
  const [contractDeposit, setContractDeposit] = useState('계약금')
  const [monthlySettlement, setMonthlySettlement] = useState('월정산')
                        ...
                        <div className="flex items-center justify-between w-full">
                          <Typography component="div">
                            <div className="flex items-center gap-2">
                              ...
                              {pageMode === 'view' ? (
                                `월고정 ${Number(monthlyFixedAmount).toLocaleString()}원`
                              ) : (
                                <TextField
                                  variant="outlined"
                                  size="small"
                                  type="text"
                                  value={`월고정 ${monthlyFixedAmount ? Number(monthlyFixedAmount.replace(/,/g, '')).toLocaleString() : ''}원`}
                                  onChange={(e) => {
                                    // "월고정 "과 "원"을 제거하고 숫자만 추출
                                    const value = e.target.value.replace('월고정 ', '').replace('원', '');
                                    const raw = value.replace(/[^0-9]/g, '');
                                    setMonthlyFixedAmount(raw);
                                  }}
                                  disabled={isViewMode()}
                                  sx={{
                                    width: '200px',
                      ...
                      <AccordionDetails>
                        ...
                        <div className="mt-4">
                          ...
                          {(() => {
                            ...
                            // useState 선언(상위로 올려야 하지만, 예시로 즉석 구현)
                            // const [checkedMonths, setCheckedMonths] = useState<string[]>([]);
                            // 파일 스코프에 선언: checkedMonths, setCheckedMonths
                            // 여기에선 상위에서 내려받았다고 가정
                            ...
                            {pageMode === 'edit' && (
                              <div className="flex items-center mt-2" style={{ gap: '8px' }}>
                                <Button
                                  ...
                                  color="primary"
                                  size="small"
                                  startIcon={<Icons.PlusIcon size={16} />}
                                >
                                  추가
                                </Button>
                              </div>
                          ...
                          {pageMode === 'edit' && (
                            <div className="flex items-center justify-between">
                              <div style={{ display: 'flex', alignItems: 'center', marginTop: 4, marginBottom: 0 }}>
                                ...
                                <RadioGroup row defaultValue="반올림" name="vatRoundType" style={{ marginLeft: 16 }}>
                                  <FormControlLabel value="반올림" control={<Radio disabled={false} />} label="반올림" />
                                  <FormControlLabel value="내림" control={<Radio disabled={false} />} label="내림" />
                                  <FormControlLabel value="올림" control={<Radio disabled={false} />} label="올림" />
                                </RadioGroup>
